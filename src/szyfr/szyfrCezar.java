...
public class szyfrCezar {
	...
	if(isLetter(tekst.charAt(i)))
		...
		wyjsciowy+=(char)((tekst.charAt(i)-'A'+p)%26+'A');
		else
		wyjsciowy+=(char)((tekst.charAt(i)-'a'+p)%26+'a');
	}
   else {
...
return wyjsciowy;
	...
	public static String reverseCezar(String tekst) {
		...
		for(int i=0;i<tekst.length();i++) {
			...
			else if (isUpper(tekst.charAt(i)))
			{
				zaszyfrowany+=(char)('Z'-('Z'-tekst.charAt(i)+ 3)%26);
			}
...
<<<<<<< HEAD
System.out.println(reverse_Cezar(Cezar(st,3)));
=======
System.out.println(reverseCezar(Cezar(st,5)));
>>>>>>> branch 'master' of https://github.com/louAphasia/cloudapp
}
}
